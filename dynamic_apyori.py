...
def gen_support_records(transaction_manager, min_support, **kwargs):
    ...
    if(rank == 0):
        while(candidates):
            ...
            for i in range(1, size):
                if len(candidates) != 0:
                    task_num += 1
                    a = candidates.pop()
                    # print("send", a, "to rank" ,i)
                    comm.send(a, dest=i, tag=Work)
                #     print("send finish")
            ...
            if max_length and length > max_length:
                for i in range(1, size):
                    comm.send(0, dest=i, tag=terminate)
                break
        ...
        while(1):
            relation_candidate = comm.recv(source=0, status=status)
            # print(rank, 'recv', relation_candidate)
...
if __name__ == '__main__':
    ...
    for i in range(len(data)):
        transactions.append(map(int, data[i].strip().split(',')))
    ...
    if rank == 0:
        ...
        for e in results:
            ...
            items = (map(int, items))
            items = sorted(items)
            r.append((items, e.support))
        r.sort(key=lambda e: (len(e[0]), e[0]))
        ans = '\n'.join([str(i) for i in r])
        with open('dyn_mpi_output.dump', 'wb') as fp:
            pickle.dump(ans, fp)
        # print(ans)
